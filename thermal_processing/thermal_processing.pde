/*
  actions:
 up-arrow: "what's on twitter"
 down-arrow: run the animation
 enter: clear letters
 other: collect (4) letters to display
...
void setup() {
  ...
  println("loading font 2");
  // this one not available on linux/ubuntu:
...
void keyPressed() {
  ...
  if (keyCode == ENTER) {
    infoint=0;
    l="";
    println("createStuff reset infoint...");
    createStuff();
  }
